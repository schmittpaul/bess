...
u_int32_t PAnonymizer::anonymize(u_int32_t orig_addr) {
  ...
  for (pos = 0; pos < 32; pos++) {
    u_int32_t mask = (unsigned int)-1 << (32 - pos);
    u_int32_t newpad =
        (first4bytes_pad << pos) | (first4bytes_pad >> (32 - pos));
    if (pos == 0) {
    ...
    *(u_int32_t*)rin_input = htonl(newpad ^ (orig_addr & mask));
    ...
    // rin_output is used.
    if (m_crypto.EncryptUpdate(rin_output, outlen, rin_input, m_blocksize) <
        0) {
      return 0;
    }
...
u_int32_t PAnonymizer::deanonymize(u_int32_t fake_addr) {
  u_int8_t rin_output[m_blocksize];
  u_int8_t rin_input[m_blocksize];
  ...
  for (pos = 0; pos < 32; pos++) {
    int mask = (unsigned int)-1 << (32 - pos);
    u_int32_t newpad =
        (first4bytes_pad << pos) | (first4bytes_pad >> (32 - pos));
    if (pos == 0) {
      mask = 0;
...
static u_int32_t reverse_bits(u_int32_t addr) {
  u_int32_t tmp = 0;
  int i;
 ...
 */
u_int32_t PAnonymizer::nonpa_anonymize(u_int32_t orig_addr) {
 ...
 */
u_int32_t PAnonymizer::nonpa_deanonymize(u_int32_t fake_addr) {
